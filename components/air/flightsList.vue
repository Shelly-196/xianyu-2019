<template> 
  <div class="FlightList"  @click="isShow=!isShow">
    <div>
      <!-- 显示的机票信息 -->
      <el-row 
      type="flex" 
        ...
        <el-col :span="20">
          <el-row type="flex" justify="space-between" align="middle" class="flight-sell" v-for="(item,index) in data.seat_infos" :key="index">
            ...
            <el-col :span="3" class="choose-button">
              <el-button @click="handleChooseTicket(data.id,item.seat_xid)" type="warning" size="mini">选定</el-button>
...
export default {
    ...
    computed:{
      rankTime(){
        ...
        let rank = end - start
        if(rank<0){
          rank = end+24*60-start
        }
        // 返回‘2时20分’的格式
        return `${Math.floor(rank/60)}时${rank%60}分`
      }
    },
    methods:{
      handleChooseTicket(id,seat_xid){
        this.$router.push({
          path:'/air/order',
          query:{
            id,
